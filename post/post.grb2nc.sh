...
for d in $(ls $rdir );do
	...
	if [ $code -eq 308 ] ; then
	wgrib2 foo.grb2 -for 308:309 -new_grid_interpolation bilinear -new_grid_winds earth -new_grid latlon $lo0:$nx:$dx $la0:$ny:$dy foo1.grb2
	else
	wgrib2 foo.grb2 -d $code -new_grid_interpolation bilinear -new_grid_winds earth -new_grid latlon $lo0:$nx:$dx $la0:$ny:$dy foo1.grb2
	fi


	wgrib2 foo1.grb2 -append -netcdf $o
