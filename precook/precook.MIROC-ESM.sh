...
#############
#
# Crea grib files para periodos de 6dias. Desde d1 a d2
#
#############
...
d2=`date +%Y-%m-%d -d "$d1 6 day"`	# Final SHORT
...
if [ -f ../out/wrfinput/$mdl.$s.$dtx.grb ];then echo 'Grib file '$mdl.$s.$dtx'.grb already exists';exit;fi
...
isfile=`ls ../files/ | grep tslsi_ | wc -l`
if [ $isfile -eq 0 ]; then
    tslsi=ts
else
    tslsi=tslsi
...
f=`ls ../files/${v}_*_${mdl}_${s}_${ens}_$y1.nc`
freq=`echo $f | awk -F_ '{print $2}'`

if [ $(($y1+1)) -eq $y2 ] && [[ $freq != *"mon"* ]];then
	cdo -s cat ../files/${v}_${freq}_${mdl}_${s}_${ens}_$y1.nc ../files/${v}_${freq}_${mdl}_${s}_${ens}_$y2.nc $scratch/crop/$v.cat.nc
...
if [ $v == tos ];then
    ...
    if [[ $freq == *"6hr"* ]];then
        ...
        for m in $mx; do
			if [ $m -gt 12 ];then
				m2=$(($m-12))
				f2=`ls ../files/${v}_${freq}_${mdl}_${s}_${ens}_$(($y1+1)).nc`
...
if [ $v == ta ] || [ $v == ua ] || [ $v == va ];then
    for freq in `ls ../files/${v}_*_${mdl}_${s}_${ens}_$y1.nc | awk -F_ '{print $2}'`; do
	    echo 'select ... '$v $freq
        f=../files/${v}_${freq}_${mdl}_${s}_${ens}_$y1.nc
...
for v in mrlsl tsl ; do
    ...
    if [[ $freq == *"6hr"* ]];then
        ...
        rm -f $scratch/crop/$v.foo.nc
        cdo3 -s -r cat $scratch/crop/$v.mon*.nc $scratch/crop/$v.foo.nc
        cdo3 -s -r  -interpolate,$scratch/crop/ps.nc -seldate,$d1,$d2 -inttime,$d1,00:00,6hour -selvar,$v $scratch/crop/$v.foo.nc $scratch/crop/$v.nc
    else
...
#done
#cdo3 -s -r merge $scratch/zinter/$v.*.nc $scratch/zinter/$v.foo.nc
...
cdo3 -s -r -f grb -setzaxis,$scratch/tab/paxis -setltype,$typ -chparam,-1,$prm -selvar,$v $scratch/ready/$v.nc $scratch/grb/$v.grb
done
...
prm=$(grep -w $v param.tab | awk '{print $2}')
typ=$(grep -w $v param.tab | awk '{print $3}')
cdo3 -s -r -f grb -setzaxis,$scratch/tab/zaxis2m -setltype,$typ -chparam,-1,$prm -selname,$v $scratch/crop/$v.nc $scratch/grb/$v.grb
done
...
prm=$(grep -w $v param.tab | awk '{print $2}')
typ=$(grep -w $v param.tab | awk '{print $3}')
cdo3 -s -r -f grb setltype,$typ -chparam,-1,$prm -selname,$v $scratch/crop/$v.nc $scratch/grb/$v.grb
done

echo "... tos"
cdo -s -r min -selvar,$tslsi $scratch/crop/$tslsi.nc -selvar,tos $scratch/crop/tos.nc $scratch/crop/tsk.nc
cdo3 -s -r -f grb -setltype,1 -chparam,-1,11 $scratch/crop/tsk.nc $scratch/grb/tsk.grb
...
cdo3 -s -r -f grb -setzaxis,$scratch/tab/soilzaixis -setltype,112 -chparam,-1,144 -mulc,$c -sellevidx,$l $scratch/zinter/soilm.nc  $scratch/grb/soilm.$l.grb
cdo3 -s -r -f grb -setzaxis,$scratch/tab/soilzaixis -setltype,112 -chparam,-1,11  -sellevidx,$l $scratch/crop/tsl.nc  $scratch/grb/soilt.$l.grb
done
...
cdo3 -s -r -f grb -setltype,$typ -chparam,-1,$prm $scratch/crop/lsmask.nc $scratch/merge/lsmask.grb
...
cdo1 -s settaxis,$d0,00:00,6hour $scratch/merge/all.grb ../out/wrfinput/$mdl.$s.$dtx.grb
